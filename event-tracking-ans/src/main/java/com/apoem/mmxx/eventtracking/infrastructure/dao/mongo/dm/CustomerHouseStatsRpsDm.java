 ...
 * <p>Name: CustomerHouseStatsRpsDm </p>
 * <p>Description:  </p>
...
@Repository
@Slf4j
@TaskLabel(name = "维度[城市-客户-条口]客户访问房源日统计", desc = "[RPS_HUS]日周期，统计转发房源量", order = 1)
public class CustomerHouseStatsRpsDm extends AbstractMapReduceDmDaoV2<CustomerHouseActionStatsEntity, MrCustomerHouseActionStatsRo, ConsumerAcqDailyDwEntity> {
    ...
    public Query sourceDataCondition(LocalDateTime localDateTime) {
        return Query.query(new Criteria().orOperator(
                Criteria.where(ProFieldEnum.ACTION_DEFINITION
                        .getName()).all(ActionDefinitionEnum.RPS_HUS.getName())
        ));
    }
