...
@Repository
@Slf4j
@TaskLabel(name = "维度[城市-客户]客户访问资讯日统计", desc = "[VIS_IFO]日周期，统计访问资讯量、访问资讯数", order = 1)
public class CustomerInfoStatsVisDm extends AbstractMapReduceDmDaoV2<CustomerInfoActionStatsEntity, MrCustomerInfoActionStatsRo, ConsumerAcqDailyDwEntity> {
    ...
    public Query removeHistoryCondition(LocalDateTime localDateTime) {
        return Query.query(new Criteria().andOperator(
                ...
                Criteria.where(ProFieldEnum.ACTION_TYPE.getName()).is(ActionTypeEnum.VIS.getName())));
    }

    @Override
    public Query sourceDataCondition(LocalDateTime localDateTime) {
        return Query.query(new Criteria().orOperator(
                Criteria.where(ProFieldEnum.ACTION_DEFINITION.getName()).all(ActionDefinitionEnum.VIS_IFO.getName())
        ));
    }
    ...
    public CustomerInfoActionStatsEntity convertToTargetData(LocalDateTime localDateTime, MrCustomerInfoActionStatsRo ro) {
        ...
        entity.setActionType(ActionTypeEnum.VIS.getName());
