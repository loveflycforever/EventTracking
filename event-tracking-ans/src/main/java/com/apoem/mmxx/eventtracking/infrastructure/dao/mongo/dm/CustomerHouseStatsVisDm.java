 ...
 * <p>Name: CustomerHouseStatsVisDm </p>
 * <p>Description:  </p>
...
@Repository
@Slf4j
@TaskLabel(name = "维度[城市-客户-条口]客户访问房源日统计", desc = "[VIS_HUS]日周期，统计访问房源量、访问房源数", order = 1)
public class CustomerHouseStatsVisDm extends AbstractMapReduceDmDaoV2<CustomerHouseActionStatsEntity, MrCustomerHouseActionStatsRo, ConsumerAcqDailyDwEntity> {
    ...
    public Query sourceDataCondition(LocalDateTime localDateTime) {
        return Query.query(new Criteria().orOperator(
                Criteria.where(ProFieldEnum.ACTION_DEFINITION
                        .getName()).all(ActionDefinitionEnum.VIS_HUS.getName())));
    }
