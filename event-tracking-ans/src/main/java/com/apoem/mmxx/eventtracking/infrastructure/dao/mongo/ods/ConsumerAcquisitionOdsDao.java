...
public class ConsumerAcquisitionOdsDao extends AbstractAutoIncrIdDao<ConsumerAcquisitionOdsEntity> {
    ...
    public List<ConsumerAcquisitionOdsEntity> find(TrackPointEntity trackPoint, LocalDateTime shardingDate) {
        String tableName = mongoGeneratedKeyHandler().getTableName(shardingDate, getTypeClass());
        Query query = Query.query(Criteria
                .where("page_name").is(trackPoint.getPageName())
                .and("method_name").is(trackPoint.getMethodName())
                .and("event_type").in(trackPoint.getEventType(), StringUtils.lowerCase(trackPoint.getEventType()))
                .and("avenue").in(trackPoint.getAvenue()));
        return mongoTemplate().find(query, this.getTypeClass(), tableName);
    }
}
